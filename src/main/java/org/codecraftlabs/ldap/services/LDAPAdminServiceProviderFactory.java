...
import org.codecraftlabs.ldap.services.apacheds.AdminServiceImpl;
...
public class LDAPAdminServiceProviderFactory extends LDAPBaseServiceProviderFactory {
	...
	public ILDAPAdminService buildService() throws LDAPServiceCreationException {
		...
		if (this.getLDAPResource().getServerVendor().equals(LDAPServiceProviderType.APACHEDS_SERVICE_PROVIDER.name())) {
			try {
				service = new AdminServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource());
			...
			try {
				service = new org.codecraftlabs.ldap.services.openldap.AdminServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource(), new DistinguishedNameBuilder());
			} catch (LDAPException exception) {
				throw new LDAPServiceCreationException("Error when creating the service", exception);
			}
		} 
