...
import org.codecraftlabs.ldap.services.openldap.RetrievalServiceImpl;
...
public class LDAPRetrievalServiceFactory extends LDAPBaseServiceProviderFactory {
	...
	public ILDAPRetrievalService buildService() throws LDAPServiceCreationException {
		...
		if (this.getLDAPResource().getServerVendor().equals(LDAPServiceProviderType.APACHEDS_SERVICE_PROVIDER.name())) {
			try {
				service = new org.codecraftlabs.ldap.services.apacheds.RetrievalServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource());
			...
			try {
				service = new org.codecraftlabs.ldap.services.msad.RetrievalServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource());
			} catch (LDAPException exception) {
			...
			try {
				service = new RetrievalServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource(), new DistinguishedNameBuilder());
			} catch (LDAPException exception) {
				throw new LDAPServiceCreationException("Error when creating the service", exception);
			}
		} else {
