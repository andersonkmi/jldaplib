...
import org.codecraftlabs.ldap.services.openldap.ServerInfoServiceImpl;
...
public class LDAPServerInfoServiceProviderFactory extends LDAPBaseServiceProviderFactory {
	...
	public ILDAPServerInfoService buildService() throws LDAPServiceCreationException {
		...
		if (this.getLDAPResource().getServerVendor().equals(LDAPServiceProviderType.APACHEDS_SERVICE_PROVIDER.name())) {
			try {
				service = new org.codecraftlabs.ldap.services.apacheds.ServerInfoServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource());
			...
			try {
				service = new ServerInfoServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource(), new DistinguishedNameBuilder());
			} catch (LDAPException exception) {
				throw new LDAPServiceCreationException("Error when creating the admin service", exception);
			}
		} 
