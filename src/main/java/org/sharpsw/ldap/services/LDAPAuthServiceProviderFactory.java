...
public class LDAPAuthServiceProviderFactory extends	LDAPBaseServiceProviderFactory {
	 ...
	 * @see org.sharpsw.ldap.services.ILDAPAuthenticationService
	 */
	@Override
	public ILDAPAuthenticationService buildService() throws LDAPServiceCreationException {
		...
		if (this.getLDAPResource().getServerVendor().equals(LDAPServiceProviderType.APACHEDS_SERVICE_PROVIDER.name())) {
			try {
				service = new org.sharpsw.ldap.services.apacheds.AuthenticationServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource());
			...
			try {
				service = new org.sharpsw.ldap.services.openldap.AuthenticationServiceImpl(this.getConnectionFactory().getConnection(this.getLDAPResource()), this.getLDAPResource(), new DistinguishedNameBuilder());
			} catch (LDAPException exception) {
				throw new LDAPServiceCreationException("Error when creating the service", exception);
			}
		} 
